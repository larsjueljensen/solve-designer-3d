--------------------------------------------------------------------------------
--  Handler.......... : onChangePaint
--  Author........... : 
--  Description...... : 
--------------------------------------------------------------------------------

--------------------------------------------------------------------------------
function SliderTarget.onChangePaint ( nColor )
--------------------------------------------------------------------------------
    ...
    this.sTexture ( nColor )
    log.message ( "Target Object = ", hTarget )

    local hParent = object.getParent ( hTarget)
    log.message ( "In fnc SliderTarget.onChangePaint: ", nColor )
...
--         
    ...
    local nRoomPart = sensor.getCategoryBitAt ( hTarget, 0, 2 )
        ...
        --Set wall color based on paint thumbnail sliderthumb
        if nRoomPart == true then --Object is type wall
        --local bResult = shape.overrideMeshSubsetMaterialEffectMap0 ( this.hTargetObject ( ), 1, nColor, shape.kMapTypeTexture )
        --get thumbnail bg color
        
        object.sendEvent ( hParent, "ASPRoom_AI", "onColorPicked" )
        
        --local hClickedThumb = hud.getUnderCursorComponent( hUser )
        --local r, g, b hud.getComponentBackgroundColor( hClickedThumb )
...
--                     shape.overrideMeshSubsetMaterialEffectMap0 ( hPart, 1, nColor, shape.kMapTypeTexture )
--                     table.setAt ( tTable, 1, nColor )
--                     end
--             
...
--                         log.message ( nColor  )
--                                 
--                         shape.overrideMeshSubsetMaterialEffectMap0 ( hPart, 0, nColor, shape.kMapTypeTexture )
--                         table.setAt ( tTable, 0, nColor )
--                     end
-- 
--                     if bState_3 == true then
--                         log.message ( "User selection 1 = ", bState_3 )
--                         log.message (nColor  )
--                         shape.overrideMeshSubsetMaterialEffectMap0 ( hPart, 2, nColor, shape.kMapTypeTexture )
--                         table.setAt ( tTable, 2, nColor )
--                     end  --endif bSteate 3 
--                     
